...
import type { Resident, Paginated } from "@/types/resident";

// ⬇️ Tabel reusable yang kamu buat tadi
import ResidentsTahfidzTable, {
  type ResidentRow,
} from "@/components/ResidentsTahfidzTable";

// ⬇️ Modal “Tambah Nilai”
import InputNilaiTahfidzModal from "@/components/InputNilaiTahfidzModal";
...
export default function InputNilaiTahfidzPage() {
  ...
  const fetchResidents = () => {
    ...
    apiGet<Paginated<Resident>>("/residents", params)
      .then((res) => {
        // map ke shape ResidentRow untuk tabel
        const list: ResidentRow[] = (res.data ?? []).map((r) => ({
          id: r.id,
          name: r.name,
          usrohName: r.usrohName,
          uniresNumber: r.uniresNumber,
        }));
        setRows(list);
      })
      .catch((e: unknown) => {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 grid grid-cols-12 gap-8">
        ...
        <aside className="col-span-12 md:col-span-3">
          ...
          <nav className="mt-6 space-y-2 text-sm">
            ...
            <button
              onClick={() =>
                router.push("/dashboard/asisten-musyrif/resident-per-usroh")
              }
              className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-emerald-900 hover:bg-emerald-900/5"
            >
              Resident Per Usroh
        ...
        <section className="col-span-12 md:col-span-9">
          ...
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <select
              ...
              onChange={(e) =>
                setUsroh(
                  USROH_OPTIONS.find((o) => o.value === e.target.value) ??
                    USROH_OPTIONS[0]
                )
              }
            ...
            {/* Tombol Add Nilai di kanan */}
            <div className="sm:ml-auto">
              <button
                onClick={() => {
                  setSelected(null); // modal tanpa preselect
                  setOpenModal(true);
                }}
          ...
          {/* === TABEL (pakai komponen) === */}
          <ResidentsTahfidzTable
            rows={rows}
            loading={loading}
            onInput={(r) => {
              // preselect resident untuk modal
              setSelected({
                id: r.id,
                name: r.name,
                usrohName: r.usrohName,
      ...
      {/* === MODAL: Tambah Nilai Resident === */}
      <InputNilaiTahfidzModal
        open={openModal}
